...
void GBROM_AssignSections( void )
	...
	for( i=0; i<MAXBANKS; i+=1 )
		{
		BankFree[i]=(struct sFreeArea *)malloc(sizeof(struct sFreeArea)) ;
		if (BankFree[i]!=NULL )
			{
			if( i==0 )
				{
				...
				if( options&OPT_SMALL )
					{
					BankFree[i]->nSize=k_ROM_BANK_SIZE * 2;
					BankSize[i]=k_ROM_BANK_SIZE * 2;
					}
				else
					{
					BankFree[i]->nSize=k_ROM_BANK_SIZE;
					BankSize[i]=k_ROM_BANK_SIZE;
					}
				...
				if (g_identifyBanks)
					{
					BankFree[i]->nOrg=k_ROM_BANK_SIZE ;
					/*
					...
					if( options&OPT_SMALL )
						...
						{
						BankFree[i]->nSize=k_ROM_BANK_SIZE-1;
						BankSize[i]=k_ROM_BANK_SIZE;
						}
	...
	while( pSection )
		{
		if( pSection->oAssigned==0 &&	pSection->Type==SECT_CODE &&	pSection->nOrg==-1 &&	pSection->nBank!=-1 )
			{
...
void PSION2_AssignSections( void )
	...
	BankFree[0]=(struct sFreeArea *)malloc(sizeof(struct sFreeArea)) ;
	if (BankFree[0]!=NULL)
		{
...
void CreateSymbolTable( void )
	...
	while( pSect )
		...
		while( i-- )
			{
			if( (pSect->tSymbols[i]->Type==SYM_EXPORT) && ( (pSect->tSymbols[i]->pSection==pSect) || (pSect->tSymbols[i]->pSection==NULL)) )
				{
				if( pSect->tSymbols[i]->pSection==NULL )
					{
					sym_CreateSymbol( pSect->tSymbols[i]->pzName, pSect->tSymbols[i]->nOffset, -1 );
					}
				else
					{
					sym_CreateSymbol( pSect->tSymbols[i]->pzName, pSect->nOrg+pSect->tSymbols[i]->nOffset, pSect->nBank );
					}
