...
struct	sSection	*AllocSection( void )
{
	...
	(*ppSections)=(struct sSection *)malloc(sizeof(struct sSection)) ;
	if (*ppSections)
		{
...
struct sSymbol *obj_ReadSymbol( FILE *f )
{
	...
	pSym=(struct sSymbol *)malloc(sizeof(struct sSymbol)) ;
	if (pSym)
		{
		readasciiz( s, f );
		pSym->pzName=(char *)malloc(strlen(s)+1) ;
		if (pSym->pzName)
			{
...
struct	sSection	*obj_ReadRGB0Section( FILE *f )
{
	...
	if( (pSection->Type==SECT_CODE) || (pSection->Type==SECT_HOME) )
	{
		...
		if( pSection->nByteSize )
			{
			pSection->pData=(UBYTE *)malloc(pSection->nByteSize) ;
			if (pSection->pData)
				{
				...
				while( nNumberOfPatches-- )
					{
					pPatch=(struct sPatch *)malloc(sizeof(struct sPatch)) ;
					if (pPatch)
					{
						*ppPatch=pPatch;
						readasciiz( s, f );
						pPatch->pzFilename=(char *)malloc(strlen(s)+1) ;
						if (pPatch->pzFilename)
						{
							...
							if( (pPatch->nRPNSize=readlong(f))>0 )
								{
								pPatch->pRPN=(UBYTE *)malloc(pPatch->nRPNSize) ;
								if (pPatch->pRPN)
									{
...
void	obj_ReadRGB0( FILE *pObjfile )
{
	...
	if( nNumberOfSymbols )
	{
		tSymbols=(struct sSymbol **)malloc(nNumberOfSymbols*sizeof(struct sSymbol *)) ;
		if (tSymbols)
		{
