...
void	strupr(char *s )
{
	// should assert here if s==NULL
	while(*s )
	{
		*s=toupper(*s);
		// this is bad, you should never, ever modify a pointer parameter like this
		s+=1;
	}
}
...
void ValidateBankNumber(int bankNumber)
	{
	if (!VerifyBankNumber(bankNumber))
		{
		ErrorReport("BANK value out of range, must be between 1 and 255.") ;
		}

	}


int VerifyBankNumber(int bankNumber)
	{
	if ((bankNumber>=k_ROM_BANK_MIN+1) && (bankNumber<=k_ROM_BANK_MAX))
		{
		return (TRUE) ;
		}
...
void ValidateBankAddress(long bankAddress)
	{
	if (!VerifyBankAddress(bankAddress))
		{
		ErrorReport( "Address must be 16-bit" );
		}

	}


int VerifyBankAddress(long bankAddress)
	{
	if ((bankAddress >= k_ROM_BANK_START_ADDRESS) && (bankAddress <= k_ROM_BANK_END_ADDRESS))
		{
		return (TRUE) ;
		}
...
void ValidateSectionName(char *sectionName)
	{
	if (!VerifySectionName(sectionName))
		{
		ErrorReport("Section name contains an illegal character, \'~\'") ;
		}

	}


int VerifySectionName(char *sectionName)
	{
	char	*p ;
...
void MakeUniqueSection(char *dest)
	{
	if (!dest)
		{
		ErrorFatal("Null string passed to MakeUniqueSection") ;
		}

	sprintf(dest, "~Unique~%x", uniqueSectionCount) ;
	uniqueSectionCount++ ;
	}
...
void strrtrim(char *s)
	...
	// if empty string
	if (*s==k_END_OF_STRING)
		{
		// exit function
		return ;
		}

	// obtain pointer to end of string (this is safe because the string length can never be 0 when it gets to here)
	p=s+strlen(s)-1 ;
	// while value at pointer is whitespace
	while (isspace(*p))
		{
		// terminate string at pointer
		*p = k_END_OF_STRING ;
		// decrement pointer
		p-- ;
		}
...
void strltrim(char *s)
	...
	// if whitespace detected at start of  string
	if (p!=s)
		{
		// copy string starting at pointer to start of specified string
		strcpy(s, p) ;
		}
...
void strtrim(char *s)
	{
	// remove trailing spaces first (spaces are usually trailing a string)
	strrtrim(s) ;
		  ...
		  scanagain:
		    ...
		    if (nFloatLen == 0 && pLongestFixed == NULL)
		    {
				if (*pLexBuffer == '"')
				{
				    ...
				    if (*pLexBuffer == '\n')
						ErrorReport ("Unterminated string");
				    else
						pLexBuffer += 1;
		...
		case LEX_STATE_MACROARGS:
			{
				...
				{
					ErrorReport("INTERNAL ERROR IN YYLEX" );
					return(0 );
				}
			}
