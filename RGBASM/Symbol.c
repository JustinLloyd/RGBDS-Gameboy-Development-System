...
struct sSymbol *createsymbol (char *s)
{
    ...
    {
		ErrorFatal ("No memory for symbol");
		return (NULL);
    }
}
...
void	sym_Purge(char *tzName )
{
	...
	{
		ErrorReport("'%s' not defined", tzName);
	}
}
...
ULONG   sym_isConstDefined (char *tzName)
{
	...
	&&	(psym->nType & SYMF_DEFINED) )
	{
		...
		{
			ErrorFatal("'%s' is not allowed as argument to the DEF function", tzName);
		}
	}
	return (0);
}
...
char   *sym_GetStringValue (char *tzSym)
{
    ...
    {
		ErrorReport ("Stringsymbol '%s' not defined", tzSym);
    }
...
ULONG   sym_GetConstantValue (char *s)
{
    ...
    if ((psym=findsymbol(s,pscope))!=NULL )
    {
		...
		{
		    ErrorFatal ("Expression must have a constant value");
		}
    }
    else
    {
		ErrorReport ("'%s' not defined", s);
    }
...
ULONG   sym_GetValue (char *s)
{
    ...
    if ((psym=findsymbol(s,pscope))!=NULL )
    {
		if (psym->nType & SYMF_DEFINED )
		{
			...
			{
				ErrorReport ("'%s' is a macro or string symbol", s);
			}
		    return (getvaluefield (psym));
		}
		else
...
void    sym_AddNewMacroArg (char *s)
{
    ...
    if (i < MAXMACROARGS)
    {
		...
		ErrorReport ("A maximum of 9 arguments allowed");
}
...
void    sym_AddEqu (char *tzSym, SLONG value)
{
		...
		|| ((nPass==2)&&(sym_isDefined(tzSym)==0))	)
    {
		...
		if ((nsym=findsymbol(tzSym,NULL))!=NULL )
		{
		    ...
		    {
				ErrorReport ("'%s' already defined", tzSym);
		    }
		}
		else
...
void    sym_AddString (char *tzSym, char *tzValue)
{
    ...
    if (nsym)
    {
		if ((nsym->pMacro=(char *)malloc(strlen(tzValue)+1))!=NULL )
		    ...
		    ErrorFatal ("No memory for stringequate");
...
void    sym_AddLocalReloc (char *tzSym)
{
		...
		|| ((nPass==2)&&(sym_isDefined(tzSym)==0)) )
    {
		...
		if (pScope)
		{
		    ...
		    ErrorFatal ("Local label in main scope");
    }
}
...
void    sym_Export (char *tzSym)
{
	...
	else
	{
		...
		ErrorReport ("'%s' not defined", tzSym);
    }
    ...
    for (i = 0; i < HASHSIZE; i += 1)
    {
		...
		while (sym != NULL)
		{
		    if (sym->nType & SYMF_LOCAL)
				...
				{
				MessageReport("EXPORT: '%s' - %08lX\n", sym->tzName, getvaluefield (sym));
				}
		    else if (sym->nType & SYMF_IMPORT)
				{
				MessageReport("IMPORT: '%s'\n", sym->tzName);
				}
		    else if (sym->nType & SYMF_EQU)
				{
				MessageReport("EQU   : '%s' - %08lX\n", sym->tzName, getvaluefield (sym));
				}
		    else if (sym->nType & SYMF_SET)
				{
				MessageReport("SET   : '%s' - %08lX\n", sym->tzName, getvaluefield (sym));
				}
		    else
				{
				MessageReport("SYMBOL: '%s' - %08lX\n", sym->tzName, getvaluefield (sym));
				}
